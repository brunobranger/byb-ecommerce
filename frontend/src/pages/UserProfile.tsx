...
// ─── Mock data — Se mantienen para las compras y direcciones ─────────────────
...
const PersonalInfo = ({ user }: { user: User }) => {
    ...
    // Sincronizar el formulario si el usuario cambia en el context
    useEffect(() => {
        setForm(user)
    }, [user])

    const field = (label: string, key: keyof User, disabled = false) => (
        ...
        <div className="space-y-8">
            <section className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    {field('Número de cliente', 'clientNumber', true)}
                    {field('Apellido y Nombre', 'fullName')}
                    {field('Correo electrónico', 'email', true)}
                    {field('Número telefónico', 'phone')}
                    {field('Clase fiscal', 'claseFiscal')}
                    {field('Tipo de documento', 'tipoDocumento')}
                    {field('DNI', 'dni')}
                </div>
                {editing && (
                    <div className="mt-6 flex justify-end">
                        <button
                            ...
                            className="bg-blue-900 text-white text-sm font-semibold px-5 py-2 rounded-xl"
                        >
                            Guardar cambios
                        </button>
                    </div>
            ...
            <section className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                <h3 className="text-lg font-bold tracking-tight mb-6">Datos de Facturación</h3>
